...
try {
    ...
    $query = $CI->db->get();
    $ciNotifications = $query->result_array();
    
    echo "🔍 CodeIgniter query results: " . count($ciNotifications) . " notifications\n";
    foreach ($ciNotifications as $notification) {
        echo "  ID: {$notification['id']}, Title: {$notification['title']}, Created: {$notification['created_at']}\n";
    }
