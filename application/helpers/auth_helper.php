...
/**
 * Require Main Admin role (IT Department with full system control)
 * @param CI_Controller $controller
 * @return array|void
 */
function require_main_admin($controller) {
    $user_data = require_auth($controller);
    if (!$user_data) return;
...
/**
 * Require Chairperson role (Sub-admin with program-specific access)
 * @param CI_Controller $controller
 * @return array|void
 */
function require_chairperson($controller) {
    $user_data = require_auth($controller);
    if (!$user_data) return;
...
/**
 * Require Main Admin or Chairperson role
 * @param CI_Controller $controller
 * @return array|void
 */
function require_admin_or_chairperson($controller) {
    $user_data = require_auth($controller);
    if (!$user_data) return;
    ...
    if (!$is_main_admin && !$is_chairperson) {
        $controller->output
            ...
            ->set_output(json_encode([
                'status' => false, 
                'message' => 'Access denied. Admin or Chairperson privileges required.'
            ]));
        return;
    }
...
/**
 * Check if current user can manage a specific program
 * @param CI_Controller $controller
 * @param string $target_program
 * @return bool
 */
function can_manage_program($controller, $target_program) {
    $user_data = check_auth($controller);
...
/**
 * Check if current user can create users of a specific role
 * @param CI_Controller $controller
 * @param string $target_role
 * @return bool
 */
function can_create_user_role($controller, $target_role) {
    $user_data = check_auth($controller);
    if (!$user_data) return false;
...
/**
 * Get user's program for access control
 * @param CI_Controller $controller
 * @return string|false
 */
function get_current_user_program($controller) {
    $user_data = check_auth($controller);
    return $user_data ? ($user_data['program'] ?? false) : false;
}
 ...
 * @return bool
 */
function is_chairperson($controller) {
    $user_data = check_auth($controller);
    return $user_data && $user_data['role'] === 'chairperson';
}
