...
class Auth extends BaseController {
    ...
    private function register_with_images() {
        try {
            ...
            // Prepare user data
            // Password is optional - if not provided, set placeholder (will be set during approval)
            $hashed_password = !empty($password) 
                ? password_hash($password, PASSWORD_BCRYPT) 
                : password_hash(uniqid('pending_', true), PASSWORD_BCRYPT); // Placeholder password
    ...
    public function verify_email() {
        ...
        $is_admin_created_chairperson = ($created_source === 'admin_created' && 
                                         $user['role'] === 'admin' && 
        ...
        if ($this->User_model->update($user['user_id'], $update_data)) {
            ...
            if ($is_bulk_upload && $bulk_temporary_password) {
                ...
                if (function_exists('send_bulk_upload_welcome_email')) {
                    try {
                        send_bulk_upload_welcome_email($user['full_name'], $user['email'], $bulk_temporary_password, $login_url);
                    } catch (Exception $e) {
                        log_message('error', 'Failed to send bulk upload welcome email: ' . $e->getMessage());
                    }
                }
                ...
                log_audit_event(
                    ...
                    'AUTHENTICATION',
                    "{$user['full_name']} verified bulk-uploaded account and received temporary credentials.",
                    [
                        'user_id' => $user['user_id'],
                        'email' => $user['email'],
                        'source' => 'bulk_upload'
